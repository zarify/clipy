...
class SmokeTests(unittest.TestCase):
    ...
    def test_sample_json_valid_and_has_runtime(self):
        ...
        self.assertTrue(
            "cdn.jsdelivr.net" in data["runtime"]["recommended"]
            or data["runtime"]["recommended"].strip() != "",
            "recommended runtime URL seems empty",
        )
        self.assertIn("feedback", data)
        self.assertIn("regex", data["feedback"])

    def test_app_js_contains_runtime_adapter(self):
        p = ROOT / "src" / "app.js"
        self.assertTrue(p.exists(), f"Missing {p}")
        txt = p.read_text(encoding="utf-8")
        self.assertIn("runtimeAdapter", txt)
        # Check for modular architecture indicators
        self.assertTrue("micropython.js" in txt or "execution.js" in txt)
